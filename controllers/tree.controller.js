...
export const updateTreeNote = async (req, res, next) => {
    ...
    const { id } = matchedData(req, { locations: ['params'] });
    const { title, content, brief, parent_id } = matchedData(req);
    ...
    try {
        ...
        const updateResult = await Tree.updateOne(
            { _id: id },
            { title: title, parent: parent_id, content: content },
            { session }
        );
        ...
        const updateResultUser = await User.updateOne(
            ...
            { $set: { 'trees.$.title': title, 'trees.$.parent_id': parent_id, "trees.$.brief": brief, "trees.$.updatedAt": Date.now() } },
            { session }
        );
